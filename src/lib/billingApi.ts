...
export function useBillingApi() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([subRes, invRes, payRes]) => {
        ...
        setSubscription(
          current ? mapSubscription(current as Record<string, unknown>) : null,
        );
        setInvoices(
          (invRes.data.invoices as Record<string, unknown>[]).map(mapInvoice),
        );
        const methods = payRes.data.payment_methods as Record<
          string,
          unknown
        >[];
        const defaultMethod =
        ...
        setPaymentMethod(
          defaultMethod ? mapPaymentMethod(defaultMethod) : null,
        );
      })
  ...
  const changePlan = useCallback(
    async (plan: string, price: string): Promise<void> => {
      const res = await portalApiClient.put(
  ...
  const cancelSubscription = useCallback(async (): Promise<void> => {
    const res = await portalApiClient.put("/v1/portal/billings/subscription", {
      cancel: true,
    });
    const current = res.data.current;
    setSubscription(
  ...
  const upsertPayment = useCallback(
    async (brand: string, last4: string, expiry: string): Promise<void> => {
      const res = await portalApiClient.put("/v1/portal/payments", {
        brand,
        last4,
        expiry,
      });
