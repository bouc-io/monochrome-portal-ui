...
class KeycloakIdentityProvider implements IdentityProvider {
    ...
    private getKeycloak(): Keycloak {
        if (!this.keycloak) {
            ...
            if (!options.url) {
                throw new Error('Keycloak URL not configured. Set VITE_SSO_SERVER_URL environment variable.');
            }
    ...
    private async performInit(): Promise<void> {
        ...
        try {
            ...
            log.info('Keycloak: Init result detailed:', {
                ...
                tokenExpired: keycloak.isTokenExpired ? keycloak.isTokenExpired() : 'method not available',
            ...
            if (keycloak.token) {
                ...
                log.info('Keycloak: Token analysis:', {
                    ...
                    isExpired: keycloak.isTokenExpired ? keycloak.isTokenExpired() : 'method not available',
                    timeSkew: keycloak.timeSkew,
            ...
            log.error('Keycloak initialization error:', {
                ...
                isCallback,
                currentUrl: window.location.href,
                stringifiedError: JSON.stringify(initError, Object.getOwnPropertyNames(initError))
            });
            ...
            if (initError?.message?.includes('fragment') || initError?.message?.includes('parsing')) {
                log.error('Fragment parsing error detected:', {
                    error: 'Fragment parsing issue',
                    currentFragment: window.location.hash,
                    suggestion: 'Check if the callback URL contains valid OAuth parameters',
    ...
    private async refreshUserToken(): Promise<void> {
        ...
        try {
            ...
            log.error('Failed to refresh Keycloak token:', {
                ...
                keycloakAuthenticated: keycloak.authenticated,
                hasRefreshToken: !!keycloak.refreshToken,
                possibleCause: 'Refresh token expired or invalid, user may need to re-login'
            });
            throw error;
        }
    }
        ...
        return {
            ...
            login: async () => {
                log.info('Initiating Keycloak login', {
                    currentUrl: window.location.href,
                    keycloakLoginUrl: keycloak.createLoginUrl ? 'Available' : 'Not available'
                });
