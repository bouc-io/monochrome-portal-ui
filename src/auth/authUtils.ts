...
export const getEnvVar = (key: string): string => {
    ...
    log.info('Environment variable check:', {
        ...
        valuePreview: value ? (value.length > 50 ? value.substring(0, 50) + '...' : value) : 'not set',
        source: window.ENV?.[key] ? 'window.ENV' : import.meta.env[key] ? 'import.meta.env' : 'default'
    });
...
export const isValidJWT = (token: string): boolean => {
    ...
    if (!isValid) {
        log.info('Token validation failed: JWT structure is invalid (should have 3 non-empty parts)');
    } else {
...
export const isKeycloakCallback = () => {
    ...
    const hasFragment = url.hash.includes('code=') || url.hash.includes('state=') || url.hash.includes('access_token=');
...
export const getIdentityProvider = (): IdentityProvider => {
    ...
    log.info('Invalid or missing VITE_IDENTITY_PROVIDER, defaulting to stub (use VITE_IDENTITY_PROVIDER=keycloak for production)', {
        configured: provider,
        fallback: 'stub'
    });
