...
import { Brain, Plus, Settings, LogOut, User, Filter, Loader2, Search } from "lucide-react";
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
...
export const MemorySidebar = ({ 
  ...
  const categories: Array<Memory['category'] | 'all'> = ['all', 'preference', 'fact', 'context', 'instruction', 'persona'];
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="w-full justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="capitalize">{filterCategory === 'all' ? 'All Categories' : filterCategory}</span>
                </div>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start" className="w-56">
              {categories.map((cat) => (
                ...
                >
                  <span className="mr-2">{cat === 'all' ? '📚' : categoryIcons[cat]}</span>
                    ...
                    <SidebarMenuItem key={memory.id} className="relative group">
                      <SidebarMenuButton 
                        className={cn(
                          ...
                          memory.importance === 'critical' ? "border-l-red-500" :
                          memory.importance === 'high' ? "border-l-orange-500" :
                          memory.importance === 'medium' ? "border-l-yellow-500" :
                      ...
                      >
                        <div className="flex items-center gap-2 w-full">
                          ...
                          <Badge 
                            variant="secondary" 
                            className={cn("text-xs capitalize", categoryColors[memory.category])}
                          >
                            {memory.category}
                          </Badge>
                          {!memory.isActive && (
