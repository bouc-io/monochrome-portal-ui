...
import { Brain, Sparkles, TrendingUp, Shield, Zap, Search, X } from "lucide-react";
...
import { Memory, useMemoryApi, CreateMemoryRequest, UpdateMemoryRequest } from "@/lib/memoryApi";
...
const Index = () => {
  ...
  const { getAllMemories, createMemory, updateMemory, deleteMemory, toggleMemoryActive, searchMemories } = useMemoryApi();
  ...
  const handleSaveMemory = async (data: CreateMemoryRequest | UpdateMemoryRequest) => {
    setIsSaving(true);
    try {
      if (editingMemory) {
        const updated = await updateMemory(editingMemory.id, data as UpdateMemoryRequest);
  ...
  useEffect(() => {
    ...
    searchDebounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        setSearchResults(
          memories.filter((m) => m.content.toLowerCase().includes(query)).map((m) => m.id)
        );
      } finally {
  ...
  const displayedMemories = memories.filter(memory => {
    ...
    const searchMatch = searchResults === null || searchResults.includes(memory.id);
    return categoryMatch && searchMatch;
  });

  // Stats for empty state
  const stats = [
    { icon: Sparkles, label: "Personalized responses", description: "AI tailored to your preferences" },
    { icon: TrendingUp, label: "Improved accuracy", description: "Better context understanding" },
    { icon: Shield, label: "Your data, your control", description: "Edit or delete anytime" },
    { icon: Zap, label: "Instant updates", description: "Changes apply immediately" },
  ];
                ...
                {isLoading ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-16">
                    ...
                    <h2 className="text-2xl font-semibold text-foreground mb-2">
                      {filterCategory !== 'all' ? 'No memories in this category' : 'No memories yet'}
                    </h2>
