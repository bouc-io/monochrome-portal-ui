...
export default function Billing() {
  const { subscription, invoices, paymentMethod, loading, changePlan, cancelSubscription, upsertPayment } = useBillingApi();
  ...
  const handleUpsertPayment = async () => {
    if (!paymentForm.brand || !paymentForm.last4 || !paymentForm.expiry) {
      toast({ title: "Validation error", description: "All payment fields are required.", variant: "destructive" });
      return;
    }
    setPaymentBusy(true);
    try {
      await upsertPayment(paymentForm.brand, paymentForm.last4, paymentForm.expiry);
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="text-sm text-muted-foreground space-y-1">
                  <p>Monthly price: <span className="font-medium text-foreground">{subscription.price}</span></p>
                  {subscription.endedAt && (
                    <p>Ended: <span className="font-medium text-foreground">{new Date(subscription.endedAt).toLocaleDateString()}</span></p>
                  )}
                </div>
                <div className="flex gap-2 pt-2">
                  <Button variant="outline" size="sm" onClick={() => setPlanDialogOpen(true)}>Change Plan</Button>
                  {subscription.status === "active" && (
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            This will cancel your current plan. You will retain access until the end of the billing period.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
            ...
            {loading ? (
              ...
              <>
                ...
                <Button variant="outline" size="sm" className="mt-2" onClick={() => {
                  setPaymentForm({ brand: paymentMethod.brand, last4: paymentMethod.last4, expiry: paymentMethod.expiry });
          ...
          <div className="space-y-2 py-2">
            {PLANS.map(({ plan, price }) => (
              <div key={plan} className="flex items-center justify-between rounded-lg border p-3">
                ...
                >
                  {planBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : subscription?.plan === plan ? "Current" : "Select"}
                </Button>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3 py-2">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setPaymentForm((f) => ({ ...f, last4: e.target.value }))}
              />
            </div>
            <div>
